...
function lzw(indices,min=8){const clear=1<<min,end=clear+1;let size=min+1,next=end+1,dict=new Map(),bits=[],cur=0,n=0;const emit=c=>{cur|=c<<n;n+=size;while(n>=8){bits.push(cur&255);cur>>=8;n-=8;}};emit(clear);let prefix=indices[0];for(let i=1;i<indices.length;i++){const k=prefix+','+indices[i];if(dict.has(k))prefix=dict.get(k);else{emit(prefix);if(next<4096){dict.set(k,next++);if(next===(1<<size)&&size<12)size++;}else{emit(clear);dict=new Map();size=min+1;next=end+1;}prefix=indices[i];}}emit(prefix);emit(end);if(n)bits.push(cur&255);return Buffer.from(bits);}
function gif(frames,file){const chunks=[Buffer.from('GIF89a','ascii')],hdr=Buffer.alloc(7);hdr.writeUInt16LE(24,0);hdr.writeUInt16LE(24,2);hdr[4]=0xF7;chunks.push(hdr);const pal=Buffer.alloc(768);for(let i=1;i<256;i++){const q=i-1;pal[i*3]=((q>>5)&7)*255/7;pal[i*3+1]=((q>>2)&7)*255/7;pal[i*3+2]=(q&3)*255/3;}chunks.push(pal,Buffer.from([0x21,0xFF,0x0B]),Buffer.from('NETSCAPE2.0'),Buffer.from([3,1,0,0,0]));for(let f=0;f<frames.length;f++){const delay=f===0?200:13;chunks.push(Buffer.from([0x21,0xF9,4,1,delay&255,delay>>8,0,0,0x2C,0,0,0,0,24,0,24,0,0,8]));const idx=[];for(let i=0;i<frames[f].length;i+=4)idx.push(paletteIndex(frames[f][i],frames[f][i+1],frames[f][i+2],frames[f][i+3]));const dat=lzw(idx);for(let p=0;p<dat.length;p+=255)chunks.push(Buffer.from([Math.min(255,dat.length-p)]),dat.subarray(p,p+255));chunks.push(Buffer.from([0]));}chunks.push(Buffer.from([0x3B]));fs.writeFileSync(file,Buffer.concat(chunks));}
